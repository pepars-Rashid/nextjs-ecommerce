...
const highlightMatches = (text: string, query: string) => {
  ...
  try {
    ...
    return parts.map((part, idx) =>
      pattern.test(part) ? (
        <mark key={idx} className="bg-yellow-200 text-dark px-0.5 rounded">{part}</mark>
      ) : (
        ...
        <div className="bg-white rounded-xl shadow-2xl ring-1 ring-black/5 overflow-hidden">
          ...
          <div className="px-4 sm:px-6 py-4 border-b border-gray-3">
            <div className="flex items-center gap-3">
              <svg width="20" height="20" viewBox="0 0 20 20" className="text-dark-3">
                <path
              ...
              >
                <svg width="20" height="20" viewBox="0 0 20 20">
                  <path d="M14.348 5.652a.833.833 0 0 0-1.18 0L10 8.82 6.832 5.652a.833.833 0 1 0-1.18 1.18L8.82 10l-3.168 3.168a.833.833 0 1 0 1.18 1.18L10 11.18l3.168 3.168a.833.833 0 1 0 1.18-1.18L11.18 10l3.168-3.168a.833.833 0 0 0 0-1.18Z" fill="currentColor" />
                </svg>
              </button>
            </div>
          </div>
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            ...
            {!loading && !error && results.length === 0 && query.trim().length > 0 && (
              <div className="px-4 sm:px-6 py-6 text-dark-4">No products match your search.</div>
            )}
            {!loading && !error && results.length > 0 && (
              <ul className="divide-y divide-gray-2">
                {results.map((p) => {
                  const imgUrl = (p.imagesArray && p.imagesArray[0]?.url) || "/images/blanck.png";
                  return (
                    <li key={p.id} className="px-4 sm:px-6 py-4 hover:bg-gray-1 ease-out duration-150">
                      <Link href={`/product/${p.productSlug || p.id}`} onClick={onClose} className="flex items-start gap-4">
                        <div className="flex-shrink-0 w-14 h-14 relative rounded-md overflow-hidden bg-gray-2">
                          <Image src={imgUrl} alt={p.title} fill sizes="56px" className="object-cover" />
                        </div>
